...
BG.Init(function()
    ...
    function BG.CreateBossNameUI(frame, FB)
        ...
        tex:SetAllPoints()
        tex:SetTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\help-i.tga")
        bt:SetHighlightTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\help-i.tga")
        ...
        for i = 1, Maxb[FB] - 2 do
            ...
            frame["Boss" .. i] = bt
            if textable[FB] and type(textable[FB][i]) == "string" then
            ...
            local scroll = CreateFrame("ScrollFrame", "BiaoGeBossSpell"..FB..i, frame, "UIPanelScrollFrameTemplate")
            scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeBossSpell"..FB..i.."ScrollBar"]
            ...
            local scroll = CreateFrame("ScrollFrame", "BiaoGeBossClass"..FB..i, frame, "UIPanelScrollFrameTemplate")
            scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeBossClass"..FB..i.."ScrollBar"]
    ...
    function BG.CreateBossSpellFrameNpc(frame, NPCNum, bossName, bossOnEnterText, Pnum)
        ...
        bt.tex:SetTexture("Interface\\Buttons\\Arrow-Down-Down")
        ...
        local function Open()
            if frame["NPCframe" .. NPCNum].height then
                if bt.open then
                    ...
                    bt.tex:SetTexture("Interface\\Buttons\\Arrow-Right-Down")
                    bt.open = nil
                else
                    local i = 1
                    while frame["NPCframe" .. NPCNum]["Spellname" .. i] do
    ...
    function BG.CreateBossSpell(frame, NPCNum, SpellNum, spellID, SpellInfoText, SpellDoneText, Pnum, spellID2, spellID3)
        ...
        local icon2 = spellID2 and GetSpellTexture(spellID2)
        local icon3 = spellID3 and GetSpellTexture(spellID3)
        t:SetText(AddTexture(icon) .. C_Spell.GetSpellLink(spellID)
            .. (spellID2 and (AddTexture(icon2) .. C_Spell.GetSpellLink(spellID2)) or "")
            .. (spellID3 and (AddTexture(icon3) .. C_Spell.GetSpellLink(spellID3)) or ""))
        local w = t:GetStringWidth()
        f:SetWidth(w)
        -- 阶段文字（P1、P2、全程）
        local p_t = f:CreateFontString()
        ...
        BG.SafeHyperlinkScript(f, "OnHyperlinkClick", function(self, link, text, button)
        ...
        BG.SafeHyperlinkScript(f, "OnHyperlinkLeave", GameTooltip_Hide)
    ...
    function BG.CreateBossSpellTisIcon(frame, NPCNum, SpellNum, IconNum, NPC)
        ...
        icon:SetAllPoints()
        -- FileDataID 521749 is a Retail-only sprite sheet; WoW 3.3.5 needs path-based textures.
        local _iconPaths = {
            [1]  = "Interface\\Icons\\Ability_Warrior_DefensiveStance", -- tank warning
            [2]  = "Interface\\Icons\\Ability_DualWield",               -- dps warning
            [3]  = "Interface\\Icons\\Spell_Holy_HolyBolt",             -- heal warning
            [4]  = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_1", -- heroic (star)
            [5]  = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_8", -- wipe skill (skull)
            [6]  = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_3", -- important (diamond)
            [7]  = "Interface\\Icons\\Ability_Kick",                    -- interruptible
            [8]  = "Interface\\Icons\\Spell_Arcane_Arcane01",           -- spell effect
            [9]  = "Interface\\Icons\\Spell_Shadow_Curse",              -- curse
            [10] = "Interface\\Icons\\Ability_Poisons",                 -- poison
            [11] = "Interface\\Icons\\Spell_Nature_RemoveDisease",      -- disease
        }
        icon:SetTexture(_iconPaths[IconNum] or "Interface\\Icons\\INV_Misc_QuestionMark")
    end
