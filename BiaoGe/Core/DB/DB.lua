...
local LibBG = ns.LibBG  -- C_UIDropDownMenu adapter (set by Core/DropDownAdapter.lua)
...
BG.Init(function()
    ...
    do
        ...
        if not BG.IsRetail then
            ...
            function BG.GetTalentIcon(class, talent, w)
                ...
                local tex = BG.classIcon and BG.classIcon[class] or "Interface\\Icons\\INV_Misc_QuestionMark"
                local a, b, c, d = unpack(BG.iconTexCoord)
                local coord = format("100:100:%s:%s:%s:%s", a * 100, b * 100, c * 100, d * 100)
                return format("|T%s:%s:%s:0:0:%s|t", tex, w, w, coord)
            end
            ...
            BG.classIcon = {
                DEATHKNIGHT = BG.talentIcon.DEATHKNIGHT[1],
                PALADIN = BG.talentIcon.PALADIN[2],
                WARRIOR = BG.talentIcon.WARRIOR[3],
                SHAMAN = BG.talentIcon.SHAMAN[1],
                HUNTER = BG.talentIcon.HUNTER[1],
                DRUID = BG.talentIcon.DRUID[1],
                ROGUE = BG.talentIcon.ROGUE[1],
                MAGE = BG.talentIcon.MAGE[1],
                WARLOCK = BG.talentIcon.WARLOCK[1],
                PRIEST = BG.talentIcon.PRIEST[1],
            }
        end
    ...
    do
        ...
        function BiaoGe_InputBoxTemplate_OnLoad(self)
            ...
            local r, g, b = ns.GetClassRGB(nil, "player")

            self.Middle = self.Middle or self:CreateTexture(nil, "BACKGROUND")
            self.Middle:ClearAllPoints()
            self.Middle:SetPoint("TOPLEFT", -4, 2)
            self.Middle:SetPoint("BOTTOMRIGHT", 0, -2)
            self.Middle:SetColorTexture(0, 0, 0, 0.28)

            self.Left = self.Left or self:CreateTexture(nil, "BORDER")
            self.Left:ClearAllPoints()
            self.Left:SetPoint("TOPLEFT", self.Middle, "TOPLEFT")
            self.Left:SetPoint("BOTTOMLEFT", self.Middle, "BOTTOMLEFT")
            self.Left:SetWidth(1)
            self.Left:SetColorTexture(r, g, b, 0.7)

            self.Right = self.Right or self:CreateTexture(nil, "BORDER")
            self.Right:ClearAllPoints()
            self.Right:SetPoint("TOPRIGHT", self.Middle, "TOPRIGHT")
            self.Right:SetPoint("BOTTOMRIGHT", self.Middle, "BOTTOMRIGHT")
            self.Right:SetWidth(1)
            self.Right:SetColorTexture(r, g, b, 0.7)

            self.Top = self.Top or self:CreateTexture(nil, "BORDER")
            self.Top:ClearAllPoints()
            self.Top:SetPoint("TOPLEFT", self.Middle, "TOPLEFT")
            self.Top:SetPoint("TOPRIGHT", self.Middle, "TOPRIGHT")
            self.Top:SetHeight(1)
            self.Top:SetColorTexture(r, g, b, 0.7)

            self.Bottom = self.Bottom or self:CreateTexture(nil, "BORDER")
            self.Bottom:ClearAllPoints()
            self.Bottom:SetPoint("BOTTOMLEFT", self.Middle, "BOTTOMLEFT")
            self.Bottom:SetPoint("BOTTOMRIGHT", self.Middle, "BOTTOMRIGHT")
            self.Bottom:SetHeight(1)
            self.Bottom:SetColorTexture(r, g, b, 0.7)
        end
    end
...
BG.Init2(function()
    ...
    if BG.IsWLK_80 then
        ...
        if not ns.canShowTBC and BG.TabButtonsFB_TBC then
