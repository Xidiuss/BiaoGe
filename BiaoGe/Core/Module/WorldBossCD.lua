        ...
        function bt:FullUpdate(i, name, rank, class, role, combatRole, isML)
            if name then
                ...
                elseif role == "MAINTANK" then
                    ...
                    self.icon:SetTexCoord(0/64, 19/64, 22/64, 41/64)
                elseif role == "MAINASSIST" then
                    self.icon:SetTexture("Interface\\LFGFrame\\UI-LFG-ICON-ROLES")
                    self.icon:SetTexCoord(20/64, 39/64, 22/64, 41/64)
                elseif rank == 1 then
            ...
            elseif not IsInRaid(1) then
                ...
                self.talent:SetTexture(BG.classIcon and BG.classIcon[class] or "Interface\\Icons\\INV_Misc_QuestionMark")
                if BG.iconTexCoord then
                    self.talent:SetTexCoord(unpack(BG.iconTexCoord))
                end
            end
            self:SubUpdate()
        end
