...
-- IsTW = "Taiwan/non-CN server"; only meaningful for retail/MOP which have real TW realms.
-- Classic/WotLK/TBC builds return GetCurrentRegion()=1 from the Compat stub regardless of
-- actual region, so we must not set IsTW there — it would enable TW-only UI that has no
-- equivalent in 3.3.5 (Blizzard_Communities, WoW Token, etc.).
if GetCurrentRegion() ~= 5 and not (BG.IsVanilla or BG.IsTBC or BG.IsWLK or BG.IsCTM) then
    BG.IsTW = true
end
...
-- BG.SafeHyperlinkScript — pcall-guarded SetScript for hyperlink events.
-- Frame and EditBox on WotLK 3.3.5 private servers don't support
-- OnHyperlinkEnter/Leave/Click. Replacing idx.SetScript in the shared frame
-- metatable from addon code taints it globally (breaks Clique/action bars).
-- Use this at each individual call site instead.
do
    local _HL = { OnHyperlinkEnter=true, OnHyperlinkLeave=true, OnHyperlinkClick=true }
    function BG.SafeHyperlinkScript(frame, event, handler)
        if _HL[event] then
            pcall(frame.SetScript, frame, event, handler)
        else
