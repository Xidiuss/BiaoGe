...
local function AddTexture(Texture, y, coord, width)
    ...
    if Texture == "MAINTANK" then                      -- 主坦克 (WotLK: use file path; numeric 132064 is BfA+)
        return "|TInterface\\LFGFrame\\UI-LFG-ICON-ROLES:14:14:0:0:64:64:0:19:22:41|t"
    elseif Texture == "MAINASSIST" then                -- 主助理 (WotLK: use file path; numeric 132063 is BfA+)
        return "|TInterface\\LFGFrame\\UI-LFG-ICON-ROLES:14:14:0:0:64:64:20:39:22:41|t"
    elseif Texture == "TANK" then                      -- 坦克职责 (UI-LFG-ICON-ROLES 64x64: left=0,right=19,top=22,bottom=41)
        return "|TInterface\\LFGFrame\\UI-LFG-ICON-ROLES:14:14:0:0:64:64:0:19:22:41|t"
    elseif Texture == "HEALER" then                    -- 治疗职责 (UI-LFG-ICON-ROLES 64x64: left=20,right=39,top=1,bottom=20)
        return "|TInterface\\LFGFrame\\UI-LFG-ICON-ROLES:14:14:0:0:64:64:20:39:1:20|t"
    elseif Texture == "DAMAGER" then                   -- 输出职责 (UI-LFG-ICON-ROLES 64x64: left=20,right=39,top=22,bottom=41)
        return "|TInterface\\LFGFrame\\UI-LFG-ICON-ROLES:14:14:0:0:64:64:20:39:22:41|t"
    elseif Texture == 135994 or Texture == "LFG_HEAD" then
...
function BG.SetButtonAtlas(bt, atlas)
    ...
    tex:SetAllPoints()
    pcall(function() tex:SetAtlas(atlas) end)
    local tex2 = bt:CreateTexture()
    tex2:SetAllPoints()
    pcall(function() tex2:SetAtlas(atlas) end)
...
-- !!!ClassicAPI-native backdrop constants.
-- BG_BACKDROP_BORDER  – thin ChatFrameBackground edge, no fill (for frames with own bg textures)
-- BG_BACKDROP_THIN    – thin edge + UI-Tooltip-Background fill (SetBackdropColor controls alpha)
-- BG_BACKDROP_PANEL   – 16px Tooltip-Border + fill (larger bordered panels)
-- BG_BACKDROP_PANEL_10 – 10px Tooltip-Border + fill (compact bordered panels)
BG_BACKDROP_BORDER = {
...
function BG.SetSafeBackground(frame, r, g, b, a)
    if not frame then return end
    if not frame.BiaoGeSafeBg then
        local tex = frame:CreateTexture(nil, "BACKGROUND")
        tex:SetAllPoints(frame)
        frame.BiaoGeSafeBg = tex
    end
    local tex = frame.BiaoGeSafeBg
    tex:SetColorTexture(r or 0, g or 0, b or 0, a or 0)
    tex:SetShown((a or 0) > 0)
    return tex
end
