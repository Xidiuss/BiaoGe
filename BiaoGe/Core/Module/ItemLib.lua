...
local function DeriveItemClassID(typeID, EquipLoc)
    if typeID then return typeID end
    if not EquipLoc or EquipLoc == "" then return end
    return weaponEquipLoc[EquipLoc] and 2 or 4
end
...
local function HideItemLibFloatingFrames()
    GameTooltip:Hide()
    if BiaoGeTooltip then
        BiaoGeTooltip:Hide()
    end
    if BiaoGeTooltip2 then
        BiaoGeTooltip2:Hide()
    end
            ...
            elseif isDoing then
                for ii = startI, startI + oneTime - 1 do
                    ...
                    if itemID then
                        ...
                        local item = type(_itemID) == "number" and Item:CreateFromItemID(_itemID)
...
do
    local function IsYesItem(itemID)
        ...
        local typeID = DeriveItemClassID(info[FB][itemID].typeID, EquipLoc)
        info[FB][itemID].typeID = typeID
    ...
    local function InsertItemInfo(FB, itemID, type, hard, ii, other)
        ...
        elseif type == "profession" then -- 专业制造
            local icon = ""
            if other == "锻造" then
                icon = AddTexture("Interface\\Icons\\Trade_BlackSmithing", nil, ":100:100:8:92:8:92")
            elseif other == "制皮" then
                icon = AddTexture("Interface\\Icons\\Trade_LeatherWorking", nil, ":100:100:8:92:8:92")
            elseif other == "裁缝" then
                icon = AddTexture("Interface\\Icons\\Trade_Tailoring", nil, ":100:100:8:92:8:92")
            elseif other == "工程" then
                icon = AddTexture("Interface\\Icons\\Trade_Engineering", nil, ":100:100:8:92:8:92")
            elseif other == "附魔" then
                icon = AddTexture("Interface\\Icons\\Trade_Engraving", nil, ":100:100:8:92:8:92")
            elseif other == "珠宝加工" or other == "珠宝" then
                icon = AddTexture("Interface\\Icons\\INV_Misc_Gem_01", nil, ":100:100:8:92:8:92")
            elseif other == "铭文" then
                icon = AddTexture("Interface\\Icons\\INV_Inscription_Tradeskill01", nil, ":100:100:8:92:8:92")
            elseif other == "考古" then
                icon = AddTexture(441139, nil, ":100:100:8:92:8:92")
            elseif other == "炼金" then
                icon = AddTexture("Interface\\Icons\\Trade_Alchemy", nil, ":100:100:8:92:8:92")
            end
...
function BG.ItemLibUI()
    ...
    if type(BiaoGe.ItemLib.ItemLibInvType) ~= "table" or not BiaoGe.ItemLib.ItemLibInvType[1] then
        BiaoGe.ItemLib.ItemLibInvType = { "INVTYPE_HEAD" }
    end
    ...
    do
        ...
        do
            ...
            mainFrame.bg = f
            local scroll = CreateFrame("ScrollFrame", "BiaoGeItemLibMainScroll", f, "UIPanelScrollFrameTemplate")
            scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeItemLibMainScrollScrollBar"]
        ...
        do
            ...
            bt.normalTex:SetTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\libi.tga")  -- "koło ze strzałek" = źródło/pozyskanie (WotLK-safe, nie mapa)
            bt.normalTex:SetTexCoord(0, 1, 0, 1)
            ...
            bt.highlightTex:SetTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\libi.tga")
            bt.highlightTex:SetTexCoord(0, 1, 0, 1)
            ...
            for i, v in ipairs(BG.itemLibGetFiter) do
                local btName = "BiaoGeItemLibFilter_" .. i
                ...
                if ii == 1 then
                    ...
                    edit:SetScript("OnEnter", function(self)
                        ...
                        if itemID then
                            ...
                            GameTooltip:ClearLines()
                            pcall(GameTooltip.SetHyperlink, GameTooltip, BG.SetSpecIDToLink(link))
