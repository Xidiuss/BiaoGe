...
BG.Init(function()
    ...
    do
        ...
        -- Anchor to `main` (= InterfaceOptionsFramePanelContainer in WotLK) so the
        -- backdrop never renders outside the Options panel. The 60px top-offset leaves
        -- room for the row of tab buttons that sit above the scroll frames.
        f:SetPoint("TOPLEFT",     main, "TOPLEFT",     0, -60)
        f:SetPoint("BOTTOMRIGHT", main, "BOTTOMRIGHT",  0,   0)
    ...
    do
        ...
        function BG.OptionsCreateTab(name, text) -- "Options_biaoge",L["表格"]
            ...
            frame:SetSize(1, 1)
            local scrollName = "BiaoGeOptScroll_" .. name
    ...
    do
        ...
        do
            ...
            function O.CreateSlider(name, text, parent, minValue, maxValue, step, x, y, ontext, width)
                ...
                -- WotLK 3.3.5 uses OptionsSliderTemplate (TextToSpeechSliderTemplate is retail-only)
                local template = "OptionsSliderTemplate"

                local sliderName = "BiaoGeSlider" .. name
                local slider = CreateFrame("Slider", sliderName, parent, template)
        ...
        do
            ...
            function O.CreateCheckButton(name, text, parent, x, y, ontext, long)
                local btName = "BiaoGeOptCheck_" .. name
                local bt = CreateFrame("CheckButton", btName, parent, "ChatConfigCheckButtonTemplate")
    ...
    do
        ...
        do
            ...
            do
                ...
                if BG.ReceiveMainFrame then BG.ReceiveMainFrame:SetScale(tonumber(BiaoGe.options[name]) * 0.95) end
            ...
            f:SetScript("OnValueChanged", function(self, value)
                ...
                if BG.ReceiveMainFrame then BG.ReceiveMainFrame:SetScale(tonumber(value) * 0.95) end
                if BG.FBCDFrame then
        ...
        do
            local name = "alpha"
            BG.options[name .. "reset"] = 0.55
            BiaoGe.options[name] = tonumber(BiaoGe.options[name]) or BG.options[name .. "reset"]
            ...
            local function UpdateAlpha(alpha)
                alpha = min(tonumber(alpha) or BG.options[name .. "reset"], 0.65)
                BG.MainFrame.Bg:Show()
                BG.MainFrame.Bg:SetAlpha(alpha)
                BG.auctionLogFrame:SetBackdropColor(0, 0, 0, 0)
                BG.itemGuoQiFrame:SetBackdropColor(0, 0, 0, 0)
                if BG.tabButtons then
                    for _, v in ipairs(BG.tabButtons) do
                        v.button.bg:SetAlpha(min(alpha, 0.55))
                    end
                end
            end
        ...
        do
            local name = "bg"
            BG.options[name .. "reset"] = "0,0,0,0.55"
            BiaoGe.options[name] = BiaoGe.options[name] or BG.options[name .. "reset"]
            if BiaoGe.options[name] == "0,0,0,0" then
                BiaoGe.options[name] = BG.options[name .. "reset"]
            end
            BG.Once("options", 250610, function()
                if BiaoGe.options[name] == "Interface/FrameGeneral/UI-Background-Rock" then
                    BiaoGe.options[name] = BG.options[name .. "reset"]
                    BiaoGe.options["alpha"] = 0.55
                end
            end)

            local table = {
                { tex = "Interface/FrameGeneral/UI-Background-Rock", name = L["岩石"], alpha = 0.55 },
                { tex = "Interface/FrameGeneral/UI-Background-Marble", name = L["大理石"], alpha = 0.55 },
                { tex = "0,0,0,0.55", name = L["黑夜"], },
            ...
            local function SetTex(v, setAlpha)
                if strfind(v, "/") or strfind(v, "\\") or not strfind(v, ",") then
                    ...
                    if setAlpha then
                        ...
                        BG.MainFrame.Bg:Show()
                        BG.MainFrame.Bg:SetAlpha(min(a, 0.65))
                    end
                else
                    local r, g, b, a = strsplit(",", v)
                    if not a then a = 0.55 end
                    a = tonumber(a)
                    local _r, _g, _b = tonumber(r) or 0, tonumber(g) or 0, tonumber(b) or 0
                    BG.MainFrame.Bg:SetColorTexture(_r, _g, _b, 1)
            ...
            BiaoGe.options["alpha"] = tonumber(BiaoGe.options["alpha"]) or 0.55
            if BiaoGe.options["alpha"] <= 0 then
                BiaoGe.options["alpha"] = 0.55
            end
            BG.MainFrame.Bg:Show()
            BG.MainFrame.Bg:SetAlpha(min(BiaoGe.options["alpha"], 0.65))
        ...
        if BG.fontList then
            ...
            for i = 1, (L_UIDROPDOWNMENU_MAXBUTTONS or 0) do
                local button = _G["L_DropDownList1Button" .. i]
                if button then
                    button:HookScript("OnEnter", function()
                        if L_DropDownList1 and L_DropDownList1.dropdown ~= dropDown then return end
                        if button.arg1 then
                ...
                if dropDown == BG.options["button" .. name] then
                    for i = 1, _G['L_DropDownList1'].numButtons do
                        ...
                        if not button.playSound then
                            ...
                            local normalTexture = bt:GetNormalTexture()
        ...
        do
            local function CreateFBCDbutton(n1, n2, collapse, tblName, dbName)
                ...
                for i = n1, n2 do
                    ...
                    local btName = "BiaoGeFBCDCk_" .. (dbName or "x") .. i
                    local bt = CreateFrame("CheckButton", btName, lastFrame.child2, "ChatConfigCheckButtonTemplate")
                    bt.Text = bt.Text or _G[btName .. "Text"]
            ...
            local function CreateTitle(name, color)
                ...
                frame.tex:SetTexture("Interface\\Buttons\\Arrow-Down-Down")
                ...
                frame:SetScript("OnMouseDown", function(self)
                    if self.open then
                        ...
                        self.tex:SetTexture("Interface\\Buttons\\Arrow-Right-Down")
                        self.open = nil
                    else
                        self.child2:Show()
                        self.child:SetHeight(self.height)
                        self.tex:SetTexture("Interface\\Buttons\\Arrow-Down-Down")
                        self.open = true
                    end
                    BG.PlaySound(1)
                end)
        ...
        BG.Init2(function()
            ...
            if mainFrame and mainFrame.titleFrame then
                ...
                do
                    ...
                    bt.frame:SetBackdropColor(0, 0, 0, 0)
    ...
    do
        ...
        do
            ...
            text:SetWidth(math.max(400, SettingsPanel.Container:GetWidth() - 70))
            ...
            local text = config:CreateFontString()
            text:SetFont(BIAOGE_TEXT_FONT, 15, "OUTLINE")
            text:SetPoint("TOPLEFT", width + width2 + 5, height - 15)
            ...
            local text = config:CreateFontString()
            text:SetFont(BIAOGE_TEXT_FONT, 15, "OUTLINE")
            text:SetPoint("TOPLEFT", width + width2 + 5 + width2 + 5, height - 15)
            text:SetText(L["操作"])
            text:SetTextColor(0, 1, 0)
        ...
        local function CreateButton(text, configName)
            local btName = "BiaoGeFilterCk_" .. configName
