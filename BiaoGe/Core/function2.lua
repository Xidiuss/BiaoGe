...
do
    ...
    function BG.Tooltip_SetItemByID(itemID)
        BiaoGeTooltip:SetAlpha(0)
        BiaoGeTooltip:SetOwner(UIParent, "ANCHOR_NONE")
        BiaoGeTooltip:ClearLines()
        BiaoGeTooltip:SetItemByID(itemID)
        if not BiaoGeTooltip.hideScheduled then
            BiaoGeTooltip.hideScheduled = true
            BG.After(0, function()
                BiaoGeTooltip.hideScheduled = nil
                BiaoGeTooltip:Hide()
            end)
        end
    end
    ...
    function BG.UpdateAllFilter()
        ...
        for k, bt in pairs(BG.ItemLibMainFrame and BG.ItemLibMainFrame.Hope or {}) do
        ...
        if BG.ItemLibMainFrame and not BG.ItemLibMainFrame:IsVisible() then
...
do
    ...
    local function SetButtonText(self)
        ...
        self.link = link
        if not link then
            self:SetText("|cff808080item:" .. tostring(itemID or self.itemID) .. "|r")
        ...
        for t = 1, 2 do
            for i = 1, MaxI do
                ...
                if bt.itemID then
                    ...
                    if item and not item:IsItemEmpty() and not GetItemInfo(itemID) then
                        item:ContinueOnItemLoad(function()
                            SetButtonText(bt)
                        end)
                    else
...
do
    ...
    function BG.SetListjine(jine, FB, b, i)
        ...
        BG.FrameJineList = f
        BG.SafeHyperlinkScript(f, "OnHyperlinkEnter", function(self, link, text, button)
        ...
        BG.SafeHyperlinkScript(f, "OnHyperlinkLeave", function(self, link, text, button)
            GameTooltip:Hide()
...
do
    ...
    function BG.CreateScrollFrame(parent, w, h, isEdit, alwaysHide)
        ...
        _scrollID = _scrollID + 1
        local scrollName = "BiaoGeScroll_" .. _scrollID
        local scroll = CreateFrame("ScrollFrame", scrollName, f, "UIPanelScrollFrameTemplate")
        scroll.ScrollBar = scroll.ScrollBar or _G[scrollName .. "ScrollBar"]
...
local blackup = CreateColor(0, 0, 0, 0)
local blackdown = CreateColor(0, 0, 0, 0)
local classColorup = CreateColor(r, g, b, 0)
local classColordown = CreateColor(r, g, b, 0)
local disColorup = CreateColor(.5, .5, .5, 0)
local disColordown = CreateColor(0, 0, 0, 0)
local borderAlpha = 1
function BG.CreateButton(parent)
    local bt = CreateFrame("Button", nil, parent, "BackdropTemplate")
    bt:SetBackdrop(BG_BACKDROP_BORDER)
    bt:SetBackdropBorderColor(0, 0, 0, borderAlpha)
    bt.bg = bt:CreateTexture(nil, "BACKGROUND")
    bt.bg:SetAllPoints()
    bt.bg:SetColorTexture(0, 0, 0, 0.35)
    ...
    hooksecurefunc(bt, "SetScript", function(arg1, arg2, ...)
        if arg2 == "OnEnter" then
            bt:HookScript("OnEnter", function()
                bt.bg:SetColorTexture(r, g, b, 0.45)
    ...
    hooksecurefunc(bt, "SetEnabled", function(arg1, arg2, ...)
        ...
        elseif arg2 == false then
            bt.bg:SetColorTexture(.5, .5, .5, 0.18)
            bt:GetFontString():SetTextColor(.5, .5, .5)
        end
    end)
    local _origEnable = bt.Enable
    local _origDisable = bt.Disable
    function bt:Disable()
        if _origDisable then _origDisable(self) end
        bt.bg:SetColorTexture(.5, .5, .5, 0.18)
        bt:GetFontString():SetTextColor(.5, .5, .5)
    end
    function bt:Enable()
        if _origEnable then _origEnable(self) end
        bt.bg:SetColorTexture(0, 0, 0, 0.35)
        bt:GetFontString():SetTextColor(1, .82, 0)
    end
...
function BG.SkinDropDown(dropDown)
    local borderAlpha = 1
    local name   = dropDown:GetName()
    local bt     = name and _G[name .. "Button"]
    local left   = name and _G[name .. "Left"]
    local middle = name and _G[name .. "Middle"]
    local right  = name and _G[name .. "Right"]
    local text   = name and _G[name .. "Text"]

    if bt     then bt:Hide()     end
    if left   then left:Hide()   end
    if middle then middle:Hide() end
    if right  then right:Hide()  end
    if text then
        text:ClearAllPoints()
        text:SetPoint("TOPLEFT", 18, -8)
        text:SetPoint("TOPRIGHT", -40, -8)
    ...
    f:SetFrameLevel(max((dropDown:GetFrameLevel() or 1) - 1, 0))
    dropDown.bg = f
    if text and text.SetDrawLayer then
        text:SetDrawLayer("OVERLAY")
    end

    if bt then
        local tex = dropDown:CreateTexture(nil, "OVERLAY")
...
function BG.CreateMainFrame()
    ...
    f:SetBackdrop(BG_BACKDROP_BORDER)
    ...
    f.titleBg:SetColorTexture(r, g, b, 0.14)
    f.titleBg:Show()
...
function BG.CreateExportFrame(title, text)
    ...
    if BG.exportFrame then
        ...
        local scroll = CreateFrame("ScrollFrame", "BiaoGeExportFrameScroll", f, "UIPanelScrollFrameTemplate") -- 滚动
        scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeExportFrameScrollScrollBar"]
...
function BG.SetEditBg(edit)
    local r, g, b = GetClassRGB(nil, "player")
    edit.Middle = edit.Middle or edit:CreateTexture(nil, "BACKGROUND")
    edit.Middle:ClearAllPoints()
    edit.Middle:SetPoint("TOPLEFT", -4, 2)
    edit.Middle:SetPoint("BOTTOMRIGHT", 0, -2)
    edit.Middle:SetColorTexture(0, 0, 0, 0.28)

    edit.Left = edit.Left or edit:CreateTexture(nil, "BORDER")
    edit.Left:ClearAllPoints()
    edit.Left:SetPoint("TOPLEFT", edit.Middle, "TOPLEFT")
    edit.Left:SetPoint("BOTTOMLEFT", edit.Middle, "BOTTOMLEFT")
    edit.Left:SetWidth(1)
    edit.Left:SetColorTexture(r, g, b, 0.7)

    edit.Right = edit.Right or edit:CreateTexture(nil, "BORDER")
    edit.Right:ClearAllPoints()
    edit.Right:SetPoint("TOPRIGHT", edit.Middle, "TOPRIGHT")
    edit.Right:SetPoint("BOTTOMRIGHT", edit.Middle, "BOTTOMRIGHT")
    edit.Right:SetWidth(1)
    edit.Right:SetColorTexture(r, g, b, 0.7)

    edit.Top = edit.Top or edit:CreateTexture(nil, "BORDER")
    edit.Top:ClearAllPoints()
    edit.Top:SetPoint("TOPLEFT", edit.Middle, "TOPLEFT")
    edit.Top:SetPoint("TOPRIGHT", edit.Middle, "TOPRIGHT")
    edit.Top:SetHeight(1)
    edit.Top:SetColorTexture(r, g, b, 0.7)

    edit.Bottom = edit.Bottom or edit:CreateTexture(nil, "BORDER")
    edit.Bottom:ClearAllPoints()
    edit.Bottom:SetPoint("BOTTOMLEFT", edit.Middle, "BOTTOMLEFT")
    edit.Bottom:SetPoint("BOTTOMRIGHT", edit.Middle, "BOTTOMRIGHT")
    edit.Bottom:SetHeight(1)
    edit.Bottom:SetColorTexture(r, g, b, 0.7)
