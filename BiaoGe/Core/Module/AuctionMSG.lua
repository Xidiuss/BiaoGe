...
BG.Init(function()
    ...
    do
        ...
        -- WoW 3.3.5: ScrollingMessageFrame doesn't expose .historyBuffer or .scrollOffset.
        -- UpdateButtonItem (and its callees) crash without them; stub the interface.
        f.historyBuffer = {
            headIndex = 0,
            elements = {},
            IsFull = function() return false end,
        }
        f.scrollOffset = f.scrollOffset or 0
        f:SetScript("OnHyperlinkEnter", function(self, link, text, button)
            ...
            local itemID = link and strsub(link, 1, 4) == "item" and GetItemInfoInstant(link)
            if itemID then
    ...
    do
        ...
        bt:SetNormalTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\scale.tga")  -- powiększ ramkę czatu (lupa z plusem)
        bt:SetHighlightTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\scale.tga")
        ...
        bt:SetSize(18, 18)
        bt:SetPoint("BOTTOM", chatbt, "TOP", 1, -2)
        bt:SetNormalTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\scale-.tga")  -- pomniejsz ramkę czatu (lupa z minusem)
        bt:SetHighlightTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\scale-.tga")
        ...
        tex2:SetAllPoints()
        tex2:SetTexture("Interface\\AddOns\\BiaoGe\\Media\\icon\\help-i.tga")
        tex2:SetTexCoord(0, 1, 0, 1)
    ...
    do
        ...
        -- usunięto zdublowany dekoracyjny przycisk „Locate Gear" (lupa, brak OnClick) — info opisuje przycisk „i" wyżej (decyzja usera, Sesja 14)
