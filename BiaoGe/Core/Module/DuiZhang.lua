...
function BG.DuiZhangUI()
    ...
    hooksecurefunc(LibBG, "ToggleDropDownMenu", function(_, _, _, dropDown)
        if dropDown == BG.DuiZhangDropDown.DropDown then
            ...
            for i = 1, (L_UIDROPDOWNMENU_MAXBUTTONS or 0) do
                local button = _G["L_DropDownList1Button" .. i]
                if button and button.deleteZhangDan then
        ...
        function bt:ShowRaidMember()
            ...
            for name, v in pairs(BiaoGe.duizhang[num].member) do
                ...
                elseif v.role == "MAINTANK" then
                    bt.icon:SetTexture("Interface\\LFGFrame\\UI-LFG-ICON-ROLES")
                    bt.icon:SetTexCoord(0/64, 19/64, 22/64, 41/64)
                elseif v.role == "MAINASSIST" then
                    bt.icon:SetTexture("Interface\\LFGFrame\\UI-LFG-ICON-ROLES")
                    bt.icon:SetTexCoord(20/64, 39/64, 22/64, 41/64)
    ...
    do
        ...
        local scroll = CreateFrame("ScrollFrame", "BiaoGeDuiZhangEditScroll", f, "UIPanelScrollFrameTemplate") -- 滚动
        scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeDuiZhangEditScrollScrollBar"]
        ...
        BG.SafeHyperlinkScript(child, "OnHyperlinkEnter", function(self, link, text, button)
        ...
        BG.SafeHyperlinkScript(child, "OnHyperlinkLeave", function(self, link, text, button)
            GameTooltip:Hide()
            BG.Hide_AllHighlight()
        end)
        BG.SafeHyperlinkScript(child, "OnHyperlinkClick", function(self, link, text, button)
