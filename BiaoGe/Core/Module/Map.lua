    ...
    BG.RegisterEvent("CHAT_MSG_ADDON", function(self, event, ...)
        ...
        if receiveStart[sender] then
            if #receiveCodes >= MAX_RECEIVE_CODES then
                ...
                wipe(receiveCodes)
                BG.SendSystemMessage(format(L["接收来自%s的地图标记失败：数据过大"], SetClassCFF(sender)))
                return
            end
            -- Check timeout
            if receiveStartTimes[sender] and (time() - receiveStartTimes[sender] > RECEIVE_TIMEOUT) then
                ...
                receiveStartTimes[sender] = nil
                wipe(receiveCodes)
                BG.SendSystemMessage(format(L["接收来自%s的地图标记超时"], SetClassCFF(sender)))
                return
            end
