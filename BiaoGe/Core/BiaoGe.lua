...
BG.Init(function()
    ...
    do
        ...
        BG.MainFrame:SetBackdrop(BG_BACKDROP_BORDER)
        ...
        BG.MainFrame.titleBg:SetColorTexture(r, g, b, 0.14)
        BG.MainFrame.titleBg:Show()

        BG.MainFrame.Bg = BG.MainFrame:CreateTexture(nil, "BACKGROUND", nil, 0)
        BG.MainFrame.Bg:SetAllPoints()
        BG.MainFrame.Bg:SetColorTexture(0, 0, 0, 0.55)
        ...
        BG.MainFrame:SetScript("OnMouseDown", function(self, button)
            BG.FrameHide(0)
        ...
        BG.MainFrame:SetScript("OnShow", function(self)
            ...
            if BG.currentTabNum then
                BG.MainFrame.Bg:SetAlpha(min(tonumber(BiaoGe.options["alpha"]) or 0.55, 0.65))
            end
        end)
    ...
    do
        ...
        do
            ...
            local bt = CreateFrame("CheckButton", "BiaoGeRoundWageCheck", BG["Frame" .. FB]["scrollFrame" .. Maxb[FB] + 2].owner, "ChatConfigCheckButtonTemplate")
            bt.Text = bt.Text or _G["BiaoGeRoundWageCheckText"]
            bt:SetSize(25, 25)
    ...
    if not BG.onlyOneHard then
        ...
        local function GetCurrentRaidDifficultyInfo()
            local diffID = GetRaidDifficultyID and GetRaidDifficultyID() or 3
            return tbl[diffID] or tbl[3]
        end
        local function AddButton(diffID)
            ...
            if GetCurrentRaidDifficultyInfo().ID == diffID then
                info.checked = true
            end
        ...
        BG.RegisterEvent("GROUP_ROSTER_UPDATE", function(self, event, ...)
            LibBG:UIDropDownMenu_SetText(dropDown, GetCurrentRaidDifficultyInfo().name)
        end)
    ...
    do
        ...
        local r, g, b = GetClassRGB(nil, "player")
        local blackup = CreateColor(.3, .3, .3, 0)
        ...
        local function SetColor(bt, isOnEnter)
            local alpha = min(tonumber(BiaoGe.options.alpha) or 0.45, 0.55)
            local r, g, b
            if isOnEnter then
            ...
            bt.bg:SetColorTexture(r, g, b, alpha)
        end
    ...
    do
        ...
        if BG.IsRetail or type(ContainerFrameItemButton_OnLeave) ~= "function" then
