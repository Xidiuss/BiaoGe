...
function BG.ReceiveUI()
    ...
    do
        ...
        do
            ...
            BG.ReceiveMainFrame:SetScript("OnMouseDown", function(self, button)
                BG.FrameHide(0)
        ...
        do
            ...
            do
                ...
                BG.ReceiveAuctionLogFrame:SetScript("OnMouseDown", function(self, button)
                    BG.ReceiveMainFrame:GetScript("OnMouseDown")(BG.ReceiveMainFrame, button)
                end)
            ...
            do
                ...
                local scroll = CreateFrame("ScrollFrame", "BiaoGeReceiveAuctionLogScroll", frame, "UIPanelScrollFrameTemplate")
                scroll.ScrollBar = scroll.ScrollBar or _G["BiaoGeReceiveAuctionLogScrollScrollBar"]
        ...
        if FB and DT and title then
            ...
            for _, bossStr in ipairs({ BG.Split("&&", log) }) do
                if bossStr and bossStr ~= "" then
                    ...
                    c2 = tonumber(c2)
                    c3 = tonumber(c3)
                    if type and zhuangbei and itemlevel and quality and bindType and maijia and jine then
                        local color = { 1, 1, 1 }
                        if c1 and c2 and c3 then
                            color = { c1, c2, c3 }
                        end
    ...
    -- 接收数据
    local MAX_RECEIVE_CODES = 1000
    local RECEIVE_TIMEOUT = 30
    local receiveStartTimes = {}
    BG.RegisterEvent("CHAT_MSG_ADDON", function(self, event, ...)
        ...
        if msg:match("^!BIAOGE!") then
            ...
            receiveStartTimes[sender] = time()
            wipe(receiveCodes)
        end
        if receiveStart[sender] then
            if #receiveCodes >= MAX_RECEIVE_CODES then
                receiveStart[sender] = nil
                receiveStartTimes[sender] = nil
                wipe(receiveCodes)
                BG.SendSystemMessage(format(L["接收来自%s的数据失败：数据过大"], SetClassCFF(sender)))
                return
            end
            -- Check timeout
            if receiveStartTimes[sender] and (time() - receiveStartTimes[sender] > RECEIVE_TIMEOUT) then
                receiveStart[sender] = nil
                receiveStartTimes[sender] = nil
                wipe(receiveCodes)
                BG.SendSystemMessage(format(L["接收来自%s的数据超时"], SetClassCFF(sender)))
                return
            end
